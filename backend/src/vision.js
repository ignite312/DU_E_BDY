...
class vision {
    ...
    async TextFromImage(problemset, imagePath) {
        try {
            ...
            const objectRegex = /\{[^{}]*\}/g;
            const matches = inputString.match(objectRegex);
            const lastObject = matches ? matches[matches.length - 1] : null;
            const lastparsedobj = JSON.parse(lastObject);
            console.log(lastparsedobj);
            return lastparsedobj;
        } catch (error) {
